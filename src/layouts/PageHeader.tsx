...
export function PageHeader() {
    ...
    return (
        ...
        <PageHeaderFirstSection hidden={showFullWithSearch}/>
        <form 
        className={`gap-4 flex-grow justify-center ${showFullWithSearch ? "flex" : "hidden md:flex"}`}>
            ...
            <div className="flex flex-grow max-w-[600px]">
                <input 
                type="search" placeholder = "Search" className="rounded-l-full border border-secondary-border shadow-inner shadow-secondary
        ...
        <div 
        className={`flex-shrink-0 md:gap-2 ${showFullWithSearch ? "hidden" : "flex"}`	}>
            <Button 
            onClick={() => setShowFullWidthSearch(true)} size= "icon" variant="ghost" className="md:hidden">
                <Search/>
            </Button>
